...
from data import generate_cl_trn_dat, amzn_preprocess, amazon_item_label
from metric import eval_reco, classification_metric
...
def main(log_dir, dat_dir, emb_dim, CL_setting, reload, save_emb):
    ...
    if not reload:
        user_hist, tmp, itemid2idx = amzn_preprocess(dat_dir + 'reviews_Electronics_5.json')

        order_hist_trn, order_hist_test, id_freq = reco_seq_construct1(user_hist, itemid2idx)
    ...
    label = amazon_item_label(dat_dir + 'metadata.json')
    ...
    # generate the tf dataset for seq reco
    BATCH_SIZE = 1024
    BUFFER_SIZE = 10000
    ...
    for ws, ns in CL_setting:
        ...
        targets, contexts, labels = generate_cl_trn_dat(user_hist, ws, ns, len(itemid2idx), save_last=False)
        ...
        # starting sequential reco tasks
        ...
        # starting downstream classficiation tasks
        feat = emb_tmp
...
if __name__ == '__main__':
    ...
    parser.add_argument("--log_dir", type=str, default="log/amazon_seq_reco/")
    parser.add_argument("--dat_dir", type=str, default="data/amazon/")
